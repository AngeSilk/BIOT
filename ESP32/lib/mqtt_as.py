...
class MQTTClient(MQTT_base):
    ...
    async def wifi_connect(self, quick=False):
        ...
        if ESP8266:
            ...
            if self._ssid is not None and self._wifi_pw is not None:
                wifi_credentials = {
                    "SalaTI_D1": "edupublica",
                    "@silteck": "prosilteck",
                    # Add more SSID and passwords as needed
                }
                for ssid, pw in wifi_credentials.items():
                    self.dprint(f"Trying SSID: {ssid}")
                    s.connect(ssid, pw)
                ...
                raise ValueError("No WiFi credentials provided.")
